<template>
  <view :class="theme_view">
    <view
      :class="
        (plugins_mourning_data_is_app ? ' grayscale' : '') +
        (is_single_page == 1 ? ' single-page-top' : '')
      "
    >
      <!-- 顶部内容、如果没有轮播和导航则使用矮的浮动导航背景样式，则使用高的背景样式 -->
      <view
        v-if="load_status == 1"
        class="home-top-nav-content pr"
        :style="
          (banner_list.length > 0 || navigation.length > 0
            ? top_content_bg_color
            : top_content_search_bg_color) + top_content_style
        "
      >
        <!-- 顶部背景图片 -->
        <view class="pa top-0 left-0 right-0">
          <image
            class="bg-img wh-auto"
            mode="widthFix"
            :src="static_url + 'nav-top.png'"
          ></image>
        </view>

        <!-- 搜索 -->
        <view
          v-if="common_app_is_header_nav_fixed == 1"
          :class="
            'search-fixed-seat ' +
            (common_app_is_enable_search == 1 ? 'nav-enable-search' : '')
          "
        ></view>
        <view
          :class="
            'pr ' +
            (common_app_is_header_nav_fixed == 1 ? 'search-content-fixed' : '')
          "
          :style="
            common_app_is_header_nav_fixed == 1
        ...
        >
          <view
            ...
            "
            :style="
              (common_app_is_header_nav_fixed == 1 ? top_content_style : '') +
              (common_app_is_header_nav_fixed == 1
          ...
          >
            <view class="home-top-nav margin-bottom-sm pr padding-right-main">
              <!-- 定位 -->
              <view
                v-if="is_home_location_choice == 1"
                class="home-top-nav-location dis-inline-block va-m single-text cr-white pr bs-bb padding-left-main padding-right-lg"
                @tap="choose_user_location_event"
              >
                ...
                <text class="va-m margin-left-xs text-size-md">{{
                  user_location.text || ""
                }}</text>
              ...
              <block v-else>
                <!-- logo/标题 -->
                <view
                  class="home-top-nav-logo dis-inline-block va-m padding-left-main"
                >
                  <block
                    v-if="
                      is_home_logo_use_text == 0 &&
                      (application_logo || null) != null
                    "
                  >
                    <image
                      :src="application_logo"
                      mode="heightFix"
                      class="home-top-nav-logo-image"
                    ></image>
                  </block>
                  <block v-else>
                    <view
                      v-if="(application_title || null) != null"
                      class="home-top-nav-logo-title cr-white single-text"
                      >{{ application_title }}</view
                    >
                  </block>
                </view>
              </block>
              ...
              <view
                v-if="
                  (right_icon_list || null) != null &&
                  right_icon_list.length > 0
                "
                class="nav-top-right-icon fr"
              >
                <block v-for="(item, index) in right_icon_list">
                  <view
                    class="item dis-inline-block cp pr"
                    :data-value="item.url || ''"
                    @tap="url_event"
                  >
                    <iconfont
                    ...
                    ></iconfont>
                    <view
                      v-if="(item.badge || null) != null"
                      class="badge-icon pa"
                    >
                      <component-badge
                        :propNumber="item.badge"
                      ></component-badge>
                    </view>
                  </view>
                </block>
            ...
            <view
              v-if="common_app_is_enable_search == 0"
              class="search-content-input padding-horizontal-main"
            >
              <!-- 是否开启搜索框前面icon扫一扫 -->
              <block v-if="is_home_search_scan == 1">
                <component-search
                  :propIsBtn="true"
                  propSize="sm"
                  :propPlaceholder="$t('customview.customview.726k7y')"
                  ...
                  --
                >
                  @onicon="search_icon_event" propIcon="icon-scan"
        ...
        <!-- 轮播 -->
        <view
          class="banner-content padding-horizontal-main"
          v-if="banner_list.length > 0"
        >
          <component-banner
            :propData="banner_list"
        ...
        <view
          ...
          class="spacing-mt"
          :class="
            load_status == 1 && (common_shop_notice || null) != null
              ? ''
              : ' spacing-mb'
          "
      ...
      <view class="content padding-horizontal-main pr">
        <!-- 商城公告 -->
        <view
          v-if="load_status == 0 && (common_shop_notice || null) != null"
          class="notice"
        >
          <uni-notice-bar
            show-icon
            scrollable
            :text="common_shop_notice"
        ...
        >
          <view
            mode="aspectFit"
            class="new-icon va-m fl cp pr divider-r"
            data-value="/pages/article-category/article-category"
            @tap="url_event"
          >
            ...
            >
              <block v-for="(item, index) in article_list" :key="index">
                <swiper-item class="single-text">
                  <text
                    class="cr-base text-size-sm cp"
                    :data-value="item.category_url"
                    @tap="url_event"
                    >[{{ item.article_category_name }}]</text
                  >
                  <text
                    class="cr-base text-size-sm margin-left-xs cp"
                    :style="
                      (item.title_color || null) != null
                        ? 'color:' + item.title_color + ' !important;'
                        : ''
                    "
                    :data-value="item.url"
                    @tap="url_event"
        ...
        <block v-if="0">
          <block v-for="(pv, pi) in plugins_sort_list" :key="pi">
            ...
            <view
              v-if="
                pv.plugins == 'homemiddleadv' &&
                (plugins_homemiddleadv_data || null) != null &&
                plugins_homemiddleadv_data.length > 0
              "
              class="plugins-homemiddleadv oh spacing-mb"
            >
              <view
                v-for="(item, index) in plugins_homemiddleadv_data"
                :key="index"
                class="item border-radius-main oh cp"
                :data-value="item.url || ''"
                @tap="url_event"
              >
                <image
                  class="dis-block wh-auto border-radius-main"
                  :src="item.images"
                  mode="widthFix"
                >
                </image>
            ...
            <view
              v-if="
                pv.plugins == 'seckill' &&
                (plugins_seckill_data || null) != null &&
                (plugins_seckill_data.data || null) != null &&
                (plugins_seckill_data.data.goods || null) != null &&
                plugins_seckill_data.data.goods.length > 0
              "
            ...
            >
              <view
                class="flex-row jc-sb align-c padding-top-main padding-horizontal-main"
              >
                <view class="flex-1">
                  <image
                    class="dis-inline-block va-m icon"
                    :src="plugins_seckill_data.data.home_title_icon"
                    mode="widthFix"
                  ></image>
                  <view class="dis-inline-block va-m margin-left-sm">
                    <component-countdown
                      :propHour="plugins_seckill_data.data.time.hours"
                      :propMinute="plugins_seckill_data.data.time.minutes"
                      :propSecond="plugins_seckill_data.data.time.seconds"
                    ></component-countdown>
                  </view>
                </view>
                <text
                  data-value="/pages/plugins/seckill/index/index"
                  @tap="url_event"
                  class="arrow-right padding-right cr-grey text-size-xs cp"
                  >{{ $t("common.more") }}</text
                >
              </view>
              <component-goods-list
                :propData="{
                  style_type: 2,
                  goods_list: plugins_seckill_data.data.goods,
                }"
            ...
            <view
              v-if="
                pv.plugins == 'activity' &&
                (plugins_activity_data || null) != null
              "
            >
              <component-activity-list
                :propConfig="plugins_activity_data.base"
                :propData="plugins_activity_data.data"
                propLocation="0"
                :propLabel="plugins_label_data"
                :propCurrencySymbol="currency_symbol"
                :propIsCartParaCurve="true"
            ...
            <view
              v-if="
                pv.plugins == 'realstore' &&
                (plugins_realstore_data || null) != null
              "
            >
              <view
                v-if="
                  (plugins_realstore_data.base.home_data_list_title || null) !=
                  null
                "
                class="spacing-nav-title flex-row align-c jc-sb text-size-xs"
              >
                <text
                  class="text-wrapper title-left-border single-text flex-1 flex-width padding-right-main"
                  >{{ plugins_realstore_data.base.home_data_list_title }}</text
                >
                <text
                  data-value="/pages/plugins/realstore/search/search"
                  @tap="url_event"
                  class="arrow-right padding-right cr-grey cp"
                  >{{ $t("common.more") }}</text
                >
              </view>
              <component-realstore-list
                :propData="{
                  ...{ data: plugins_realstore_data.data },
                  ...{ random: random_value },
                }"
              ></component-realstore-list>
            </view>
            ...
            <!-- 多商户 - 插件 -->
            <view
              v-if="pv.plugins == 'shop' && (plugins_shop_data || null) != null"
            >
              <view
                v-if="
                  (plugins_shop_data.base.home_data_list_title || null) != null
                "
                class="spacing-nav-title flex-row align-c jc-sb text-size-xs"
              >
                <text
                  class="text-wrapper title-left-border single-text flex-1 flex-width padding-right-main"
                  >{{ plugins_shop_data.base.home_data_list_title }}</text
                >
                <text
                  data-value="/pages/plugins/shop/index/index"
                  @tap="url_event"
                  class="arrow-right padding-right cr-grey cp"
                  >{{ $t("common.more") }}</text
                >
              ...
              <component-shop-list
                :propConfig="plugins_shop_data.base"
                :propData="{
                  ...{ data: plugins_shop_data.data },
                  ...{ random: random_value },
                }"
              ></component-shop-list>
            </view>
            ...
            <view
              v-if="
                pv.plugins == 'binding' &&
                (plugins_binding_data || null) != null
              "
            >
              <view
                v-if="
                  (plugins_binding_data.base.home_data_list_title || null) !=
                  null
                "
                class="spacing-nav-title flex-row align-c jc-sb text-size-xs"
              >
                <text
                  class="text-wrapper title-left-border single-text flex-1 flex-width padding-right-main"
                  >{{ plugins_binding_data.base.home_data_list_title }}</text
                >
                <text
                  data-value="/pages/plugins/binding/index/index"
                  @tap="url_event"
                  class="arrow-right padding-right cr-grey cp"
                  >{{ $t("common.more") }}</text
                >
              ...
              <component-binding-list
                :propConfig="plugins_binding_data.base"
                :propData="{
                  ...{ data: plugins_binding_data.data },
                  ...{ random: random_value },
                }"
                :propCurrencySymbol="currency_symbol"
              ></component-binding-list>
            ...
            <!-- 博客-楼层顶部 - 插件 -->
            <view
              v-if="pv.plugins == 'blog' && (plugins_blog_data || null) != null"
            >
              <component-blog-list
                :propConfig="plugins_blog_data.base"
                :propData="plugins_blog_data.data"
                propLocation="0"
              ></component-blog-list>
            </view>
            ...
            <view
              v-if="
                pv.plugins == 'magic' && (plugins_magic_data || null) != null
              "
            >
              <component-magic-list
                :propData="{
                  ...plugins_magic_data,
                  ...{ random: random_value },
                }"
        ...
        <block v-if="(data_list || null) != null && data_list.length > 0">
          ...
          <block v-else>
            <!-- 自动+手动 -->
            <view
              v-for="(floor, index) in data_list"
              :key="index"
              class="floor"
            >
              <view
              ...
              >
                <view class="title-left">
                  <text
                    class="text-wrapper title-left-border"
                    :style="'color:' + (floor.bg_color || '#333') + ';'"
                    >{{ floor.name }}</text
                  >
                ...
                <text
                  :data-value="
                    '/pages/goods-search/goods-search?category_id=' + floor.id
                  "
                  @tap="url_event"
                  class="arrow-right padding-right cr-grey cp"
                  >{{ $t("common.more") }}</text
              ...
              <view class="floor-list wh-auto oh pr">
                <block
                  v-if="(floor.goods || null) != null && floor.goods.length > 0"
                >
                  <component-goods-list
                    :propData="{ style_type: 1, goods_list: floor.goods }"
                    :propLabel="plugins_label_data"
                    :propCurrencySymbol="currency_symbol"
                    :propIsCartParaCurve="true"
                    propSource="index"
        ...
        <block v-if="0">
          <block v-for="(pv, pi) in plugins_sort_list" :key="pi">
            ...
            <view
              v-if="
                pv.plugins == 'salerecords' &&
                (plugins_salerecords_data || null) != null &&
                (plugins_salerecords_data.data || null) != null &&
                plugins_salerecords_data.data.length > 0
              "
              ...
              >
                <view class="title-left">
                  <text class="text-wrapper">{{
                    plugins_salerecords_data.base.home_bottom_title ||
                    $t("index.index.s5r784")
                  }}</text>
                  <text
                    v-if="
                      (plugins_salerecords_data.base || null) != null &&
                      (plugins_salerecords_data.base.home_bottom_desc ||
                        null) != null
                    "
                    class="vice-name margin-left-sm cr-grey-9"
                    >{{ plugins_salerecords_data.base.home_bottom_desc }}</text
                  >
                </view>
              </view>
              <view class="oh">
                <swiper
                  ...
                  :display-multiple-items="
                    plugins_salerecords_data.data.length < 6
                      ? plugins_salerecords_data.data.length
                      : 6
                  "
                  interval="3000"
                  :style="
                    plugins_salerecords_data.data.length < 6
                      ? 'height:' +
                        plugins_salerecords_data.data.length * 84.33 +
                        'rpx;'
                      : ''
                  "
                >
                  <block
                    v-for="(item, index) in plugins_salerecords_data.data"
                    :key="index"
                  >
                    <swiper-item>
                      <view class="item oh padding-vertical-main">
                        <view class="item-content single-text fl">
                          ...
                          <text v-if="(item.user.province || null) != null"
                            ><text class="padding-left-xs padding-right-xs"
                              >-</text
                            >{{ item.user.province }}</text
                          >
                        </view>
                        <view class="item-content fl">
                          ...
                          >
                            ...
                            <text class="margin-left-sm single-text">{{
                              item.title
                            }}</text>
                          </view>
                        </view>
                        <view
                          class="item-content single-text fr tr cr-grey padding-top-xs"
                        >
                          {{ item.add_time }}
                        </view>
                      </view>
        ...
        <!-- 弹屏广告 - 插件 -->
        <view v-if="0" class="plugins-popupscreen wh-auto ht-auto">
          <view class="content pr">
            <view
              class="close pa cp round padding-sm tc"
              @tap.stop="plugins_popupscreen_close_event"
            >
              <iconfont
                name="icon-close-o"
                size="28rpx"
            ...
            <image
              class="dis-block auto"
              :src="plugins_popupscreen_data.images"
              mode="widthFix"
              :data-value="plugins_popupscreen_data.images_url || ''"
              @tap="url_event"
            ></image>
          </view>
        </view>
      ...
      <component-bottom-line
        :propStatus="data_bottom_line_status"
      ></component-bottom-line>

      <!-- 版权信息 -->
      <view v-if="load_status == 0">
    ...
    <block v-if="load_status == 1">
      ...
      <component-online-service
        :propIsNav="true"
        :propIsBar="true"
        :propIsGrayscale="plugins_mourning_data_is_app"
      ></component-online-service>
...
// 状态栏高度
var bar_height = parseInt(
  app.globalData.get_system_info("statusBarHeight", 0, true)
);
// #ifdef MP-TOUTIAO
bar_height = 0;
// #endif
export default {
  ...
  methods: {
    ...
    init_config(status) {
      if ((status || false) == true) {
        this.setData({
          ...
          common_shop_notice: app.globalData.get_config(
            "config.common_shop_notice"
          ),
    ...
    init(params = {}) {
      ...
      // 请求远程数据
      uni.request({
        url: app.globalData.get_request_url("home/index"),
        method: "GET",
        data: params,
        dataType: "json",
        success: (res) => {
          ...
          if (res.data.code == 0) {
            var upd_data = {
              ...
              plugins_salerecords_data:
                (data.plugins_salerecords_data || null) == null ||
                data.plugins_salerecords_data.length <= 0
                  ? null
                  : data.plugins_salerecords_data,
              plugins_activity_data:
                (data.plugins_activity_data || null) == null ||
                data.plugins_activity_data.length <= 0
                  ? null
                  : data.plugins_activity_data,
              plugins_label_data:
                (data.plugins_label_data || null) == null ||
                (data.plugins_label_data.base || null) == null ||
                (data.plugins_label_data.data || null) == null ||
                data.plugins_label_data.data.length <= 0
                  ? null
                  : data.plugins_label_data,
              plugins_homemiddleadv_data:
                (data.plugins_homemiddleadv_data || null) == null ||
                data.plugins_homemiddleadv_data.length <= 0
                  ? null
                  : data.plugins_homemiddleadv_data,
              plugins_popupscreen_data: data.plugins_popupscreen_data || null,
              plugins_mourning_data_is_app:
                parseInt(data.plugins_mourning_data || 0) == 1,
        ...
        fail: () => {
          ...
          if (
            this.load_status == 0 ||
            (this.top_content_search_bg_color || null) == null
          ) {
    ...
    init_result_common_handle() {
      ...
      var seckill_static_url =
        app.globalData.get_static_url("seckill", true) + "app/";
      ...
      if (
        ...
        var color =
          this.banner_list &&
          this.banner_list.length > 0 &&
          (this.banner_list[0]["bg_color"] || null) != null
            ? this.banner_list[0]["bg_color"]
            : theme_color;
      ...
      this.setData({
        top_content_search_content_style:
          'background-image: url("' + static_url + 'nav-top.png");',
    ...
    plugins_popupscreen_handle() {
      if (this.plugins_popupscreen_data != null) {
        // 不存在关闭缓存或者超过间隔时间则显示
        var cv =
          parseInt(uni.getStorageSync(this.plugins_popupscreen_cache_key)) || 0;
    ...
    change_banner(color) {
      ...
      this.setData({
        top_content_bg_color:
          "background: linear-gradient(180deg, " + color + " 0%, #f5f5f5 80%);",
        top_content_search_bg_color:
